...
import { GAP_MINUTES, PUBLIC_TIME } from '@/components/woo/WooForm';
  ...
  const onFinish = async (value: OpenaiFormValues) => {
    ...
    if (fileOrigin && user) {
      try {
        ...
        await axios.post(endpoint.pro.groqAI, formData);
      } catch (error: any) {
...
export default GroqAIContentForm;
