...
export async function isMerchKingStore(
  url: string
): Promise<{ detected: boolean; reason: string }> {
  const baseUrl = url.replace(/\/+$/, "");

  // 1. Check homepage HTML for MerchKing signals
  try {
    ...
    if (typeof html === "string") {
      // Strong signal: MerchKing in page title
      const dom = new JSDOM(html);
      const { document } = dom.window;
      const title = document.querySelector("title")?.textContent || "";
      if (/MerchKing/i.test(title)) {
        return { detected: true, reason: "MerchKing found in page title" };
      }

      // Strong signal: MerchKing CDN or script references
      if (/merchking\.com|cdn\.merchking/i.test(html)) {
        return { detected: true, reason: "MerchKing CDN assets found" };
      }

      // Check for MerchKing-specific meta tags
      const metaGenerator = document.querySelector('meta[name="generator"]');
      if (/merchking/i.test(metaGenerator?.getAttribute("content") || "")) {
        return { detected: true, reason: "MerchKing generator meta tag found" };
      }

      // Check for MerchKing in body class or data attributes
      const body = document.querySelector("body");
      const bodyClass = body?.className || "";
      if (/merchking/i.test(bodyClass)) {
        return { detected: true, reason: "MerchKing body class detected" };
      }

      // Check for powered by MerchKing footer text
      if (/powered\s+by\s+merchking/i.test(html)) {
        return { detected: true, reason: "Powered by MerchKing found in page" };
      }

      // Check for MerchKing JavaScript globals
      const scriptsInline = document.querySelectorAll('script:not([src])');
      for (const s of Array.from(scriptsInline)) {
        const text = s.textContent || "";
        if (/merchking|window\.merchking/i.test(text)) {
          return { detected: true, reason: "MerchKing JavaScript globals detected" };
        }
      }
    }
  } catch {
  ...
  return { detected: false, reason: "No MerchKing signals detected" };
}

export async function fetchMerchKingProducts(
  url: string
